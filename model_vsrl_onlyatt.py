...
class RelationNetworks(nn.Module):
    def __init__(
        ...
        self.verb_lookup = nn.Embedding(self.n_verbs, mlp_hidden)


        self.n_concat = self.conv.base_size() * 2 + embed_hidden + 2 * 2
        self.init_role = nn.Sequential(
        ...
        self.g = Role_Labeller(DecoderLayer(mlp_hidden, c(attn),c(ff), 0.1), 3)
    ...
    def forward(self, image, verbs, roles):
        ...
        img_role = self.init_role(conv.view(-1, 7*7*self.conv.base_size()))
        ...
        role_verb_embd = role_verb_embd.contiguous().view(-1, self.max_role_count, self.mlp_hidden)
        image_embed_expand = img_role.expand(self.max_role_count, img_role.size(0), img_role.size(1))
        concat_vec = image_embed_expand.transpose(0,1) * role_verb_embd
        mask = self.encoder.get_adj_matrix(verbs)
        if self.gpu_mode >= 0:
            mask = mask.to(torch.device('cuda'))
        g = self.g(concat_vec, mask)

        '''if self.gpu_mode >= 0:
            torch.cuda.empty_cache()'''
        #print('no issue after g')
        g = g.view(-1, self.mlp_hidden).squeeze()
