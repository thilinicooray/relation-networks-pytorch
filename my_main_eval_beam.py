...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, eval_frequency=50):
    model.train()
    train_loss = 0
    total_steps = 0
    print_freq = 50
...
def eval(model, dev_loader, encoder, gpu_mode):
    ...
    with torch.no_grad():
        ...
        for i, (img, verb, roles,labels) in enumerate(dev_loader):
            ...
            verb_predict_ids, role_predict_ids = model.forward_eval_beam(img)
            '''loss = model.calculate_eval_loss(verb_predict, verb, role_predict, labels)
            val_loss += loss.item()'''
            top1.add_point_eval_beam(verb_predict_ids, verb, role_predict_ids, labels)
